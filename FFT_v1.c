...
int Fast_Fourier_Transform(double *y_re, double *y_im, double *x_re, double *x_im, int N)
{
	...
	} else 
	{
		...
		Fast_Fourier_Transform(y_even_re, y_even_im, x_even_re, x_even_im, N/2);	// Even FFT
		Fast_Fourier_Transform(y_odd_re, y_odd_im, x_odd_re, x_odd_im, N/2);		// Odd FFT
		...
		for(k=0;k<N/2;++k)
		{
			a = w_re*y_odd_re[k] - w_im*y_odd_im[k];	// w_n^0 = 1 + 0i
			b = w_re*y_odd_im[k] + w_im*y_odd_re[k];
			y_re[k]     = y_even_re[k] + a;				// Line 64~67 is just like line 27~30
